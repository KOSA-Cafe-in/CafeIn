...
uri="http://java.sun.com/jsp/jstl/fmt" %> <%@ page session="false" %> <%@ page language="java"
  ...
  <head>
    ...
    <style>
      ...
      .circle-btn {
        ...
        cursor: pointer;
        background: #2e6cff;
        box-shadow: 0 6px 16px rgba(46, 108, 255, 0.25);
      }
      .hidden {
        display: none;
      }
      ...
      <main class="content">
        <div class="tabs">
          <div id="tab-pending" class="tab active">
            신규·처리중 <span id="cnt-pending" class="badge">0</span>
          </div>
            ...
            <c:if test="${o.status eq 'N'}">
              <div class="order-card" id="order-${o.orderId}" data-status="N">
                <div class="order-title">
                  <div>
                    <span class="takeout"
                      ><c:out value="${o.takeout eq 'Y' ? '포장' : '매장'}"
                    /></span>
                    <span class="mute"
                ...
                <div class="total-line">
                  총 <c:out value="${sumCount}" />개 |
                  <fmt:formatNumber value="${o.totalPrice}" type="number" />원
                </div>
                <c:forEach var="it" items="${o.items}">
                  <div class="item">
                    <div class="name">
                      <c:out value="${it.menuName}" /> × <c:out value="${it.count}" />
                    </div>
                    <div class="price">
                      <fmt:formatNumber value="${it.unitPrice * it.count}" type="number" />원
                    </div>
                  </div>
                </c:forEach>
      ...
      function pollOrdersDOM() {
        fetch(CTX + '/admin/orders', { headers: { 'X-Requested-With': 'fetch' } })
          ...
          .then(function (html) {
            ...
            var newPendingIds = {};
            newPendingCards.forEach(function (c) {
              newPendingIds[c.id] = c;
            });
            var newDoneIds = {};
            newDoneCards.forEach(function (c) {
              newDoneIds[c.id] = c;
            });
            ...
            Array.prototype.slice
              .call(panelPending.querySelectorAll('.order-card'))
